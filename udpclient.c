...
int main(int argc, char *argv[]) {
    ...
    received = recvfrom(socket1, (packet *) pkt_rcv, sizeof(*pkt_rcv), MSG_WAITALL, (struct sockaddr *) &serv_addr,
                        &len);
    current = 0;
    ...
    while (received > 0) {
        ...
        if (SimulateACKLoss(ack_loss_ratio) == 0) {
            send1 = sendto(socket1, (ackpkt *) ack_packet, sizeof(ackpkt), MSG_CONFIRM,
        ...
        int check = 0;
        check = recvfrom(socket1, (packet *) pkt_rcv, sizeof(*pkt_rcv), MSG_WAITALL, (struct sockaddr *) &serv_addr,
                         &len);
    }
    fclose(message);
