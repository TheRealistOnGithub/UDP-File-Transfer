//Driver code for UDP server. Copies filepath from client, reads file, and sends it to client's out.txt file.

#include "imports.h"

void printTotals(int transmit_count, int data_count, int total_count, int drop_count, int success_count, int ack_count,
                 int timeout_count) {
    /**
     * Print totals at end
     * */
...
int main(int argc, char **argv) {
    ...
    received = recvfrom(socket1, (packet *) pkt_rcv, sizeof(*pkt_rcv), MSG_WAITALL, (struct sockaddr *) &cli_addr,
                        &len);
    ...
    while (fgets(data, DATA, file1) != NULL) {
        ...
        if (SimulateLoss(packet_loss_ratio) == 0) {
            ...
            while (!stop) {
                ...
                received = 0;
                received = recvfrom(socket1, (ackpkt *) ack_packet, sizeof(ackpkt), MSG_WAITALL,
                                    (struct sockaddr *) &cli_addr, &len);
